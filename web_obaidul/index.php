...
<?php
	$stmt = $DB_con->prepare('select UserPic from image where ParentId = 6 ');
	$stmt->execute();
		if($stmt->rowCount() > 0)
			{
				while($row=$stmt->fetch(PDO::FETCH_ASSOC))
						...
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
								<div class="card-panel grey lighten-5 z-depth-1">
									  <div class="row valign-wrapper">
										<div class="col-sm-5">
										  <?php
												//$con=mysqli_connect("localhost","root","","webobaidul");
												$con = mysqli_connect("localhost","root","","webobaidul");
												$sql = "select Image from image where HeadingId = 36 ";
											   ...
											   {
											   echo "<td>"; 
											   echo '<img src="data:image/jpeg;base64,'.base64_encode($row['Image'] ).'" height="60%" width="80%"/>';
											   echo "<br>";
											   ?><?php
											   echo "</td>";
										  ...
										  <span class="black-text spnblk">
											<span class="title">
													 <?php
																//$con=mysqli_connect("localhost","root","","webobaidul");
																$con = mysqli_connect("localhost","root","","webobaidul");
																$sql = "SELECT HeadingDesc FROM Heading WHERE HeadingId =36";
																$result = $con->query($sql);
									  ...
									  <div class="row valign-wrapper">
										<div class="col-sm-5">
										  <?php
												//$con=mysqli_connect("localhost","root","","webobaidul");
												$con = mysqli_connect("localhost","root","","webobaidul");
												$sql = "select Image from image where HeadingId = 35 ";
												$result = $con->query($sql);
											   echo "<table>";
											   echo "<tr>";
										  ...
										  <span class="black-text spnblk">
											<span class="title">
													 <?php
																//$con=mysqli_connect("localhost","root","","webobaidul");
																$con = mysqli_connect("localhost","root","","webobaidul");
																$sql = "SELECT HeadingDesc FROM Heading WHERE HeadingId =35";
																$result = $con->query($sql);
...
?><i class="ion-android-more-vertical right"></i></span>
								...
								<div class="card-reveal">
								  <span class="card-title grey-text text-darken-4"><?php
									//$con=mysqli_connect("localhost","root","","webobaidul");
									$con = mysqli_connect("localhost","root","","webobaidul");
									$sql = "SELECT HeadingTitle FROM description WHERE HeadingId =34";
									$result = $con->query($sql);

								   while($row = $result->fetch_assoc())
								   {

									echo "<h3>".$row['HeadingTitle']."</h3>";
								   }
								...
								?><i class="ion-close right"></i></span>
								  <p><?php
									//$con=mysqli_connect("localhost","root","","webobaidul");
									$con = mysqli_connect("localhost","root","","webobaidul");
									$sql = "SELECT ContentDesc FROM description WHERE HeadingId =34";
									$result = $con->query($sql);
                        ...
                        <div class="container">
                            <div class="screenshoot-area">
                                <div id="screenshoot_slider" class="screenshoot-slider">
									...
									<?php
	$stmt = $DB_con->prepare('select UserPic from image where HeadingId = 37 ');
	$stmt->execute();
		if($stmt->rowCount() > 0)
			{
				while($row=$stmt->fetch(PDO::FETCH_ASSOC))
